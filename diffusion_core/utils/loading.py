...
def load_models(model_id,
    ...
    ldm_stable = StableDiffusionPipeline.from_pretrained(model_id, scheduler=scheduler).to('cpu', dtype=dtype)
    # ------------------------------------------------------------
    ...
    if forward_checkpoint is not None:
        ...
        forward_cons_model.unet = forward_cons_model.unet.to(device)
    else:
