...
def guided_step(noise_prediction_text,
    ...
    # print('NEW SCALE ', new_guidance_scale)
...
class Generator:
    ...
    def get_noise_pred(self,
                       ...
                       tau1=0.4,
                       tau2=0.6):
        latents_input = torch.cat([latent] * 1)
        # latents_input[1] = latents_input[1].detach()
        if context is None:
    ...
    def image2latent(self, image):
        # print('source_img means:', image[0].mean(), image[1].mean(), image[2].mean())
        # print('source_img shape:', image.shape)
        # print('dtypes: ', self.model.vae.dtype, self.model.dtype)
        with torch.no_grad():
            ...
            if type(image) is torch.Tensor and image.dim() == 4:
                ...
                image = image.permute(0, 3, 1, 2).to(self.model.device, dtype=self.model.dtype)
    ...
    def cons_inversion(self,
        ...
        latent = self.image2latent(image)
        # print("!!!!!!LATENT NORM!!!!!!!", torch.norm(latent))
        ...
        # print("TEST CONTEXT SHAPE", torch.norm(latent))
        for i, (t, s) in enumerate(tqdm(zip(self.forward_timesteps, self.forward_boundary_timesteps))):
            ...
            noise_pred = self.get_noise_pred(
                ...
                latent=latent,
                t=t.to(self.model.device),
                context=self.context[1].unsqueeze(0),
                guidance_scale=guidance_scale,
                w_embed_dim=w_embed_dim,
                dynamic_guidance=False)
            
            # print("NOISE_PRED NORM", i, torch.norm(noise_pred))
            latent = predicted_origin(
                noise_pred,
                torch.tensor([t] * len(latent), device=self.model.device),
                torch.tensor([s] * len(latent), device=self.model.device),
        ...
        return image_rec, latents#[latent]
