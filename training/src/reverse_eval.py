...
def log_validation(
    ...
    if args.discrete_w is not None:
        guidance_scale = 19.0 # by default, perform evaluation for CFG=7.0 (8.0 in the classical formulation). TODO: add to args.
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            ...
            for log in image_logs:
                ...
                formatted_images = np.stack(formatted_images)
                tracker.log({log["validation_prompt"]: wandb.Image(np.hstack(formatted_images), caption=validation_prompt)}, step=step)
        # else:
