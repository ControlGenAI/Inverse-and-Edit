...
def forward_alignment_step(
    ...
    alpha_schedule = torch.sqrt(noise_scheduler.alphas_cumprod).to(accelerator.device)
    sigma_schedule = torch.sqrt(1 - noise_scheduler.alphas_cumprod).to(accelerator.device)
    start_latent = latents.clone()
    bs = latents.shape[0]

    start_steps = [
        torch.Tensor([0] * bs).to(accelerator.device).long(),
        torch.Tensor([259] * bs).to(accelerator.device).long(),
        torch.Tensor([519] * bs).to(accelerator.device).long(),
        torch.Tensor([779] * bs).to(accelerator.device).long(),
    ]
    steps = [
        torch.Tensor([259] * bs).to(accelerator.device).long(),
        torch.Tensor([519] * bs).to(accelerator.device).long(),
        torch.Tensor([779] * bs).to(accelerator.device).long(),
        torch.Tensor([999] * bs).to(accelerator.device).long()
    ]

    w_zero = torch.zeros_like(w)
    w_zero = w_zero.to(device=latents.device, dtype=latents.dtype)
    if args.embed_guidance:
        w_embedding = guidance_scale_embedding(
            w.flatten().cpu().float(), embedding_dim=512
        )
        ...
        w_zero_embedding = guidance_scale_embedding(
            w_zero.flatten().cpu().float(), embedding_dim=512
        )
        w_zero_embedding = w_zero_embedding.to(device=latents.device, dtype=latents.dtype)
    ...
    for i in range(4):
        noise_pred = unet(
            latents,
            start_steps[i],
            timestep_cond=w_zero_embedding,
            encoder_hidden_states=prompt_embeds.float(),
            added_cond_kwargs=encoded_text,
        ).sample
    ...
    for i in range(4):
        ...
        noise_pred = back_unet(
            latents,
            start_steps[i],
            timestep_cond=w_embedding if i != 0 else w_zero_embedding,
            encoder_hidden_states=prompt_embeds.float(),
            added_cond_kwargs=encoded_text,
        ).sample
        latents = predicted_origin(
    ...
    # Calculate loss
    if args.loss_type == "l2":
        loss = F.mse_loss(latents.float(), start_latent.float(), reduction="mean")
    elif args.loss_type == "huber":
        loss = torch.mean(
            torch.sqrt(
                (latents.float() - start_latent.float()) ** 2 + args.huber_c**2
            ) - args.huber_c
        )
        ...
        [
            vae.decode(
                latents[i : i + 16].to(torch.float32) / vae.config.scaling_factor,
                return_dict=False,
            )[0]
        ...
        with torch.no_grad():
            ...
            [
                vae.decode(
                    start_latent[i : i + 16].to(torch.float32) / vae.config.scaling_factor,
                    return_dict=False,
                )[0]
                for i in range(0, start_latent.shape[0], 16)
            ], dim=0,
            )
        ...
        with torch.cuda.amp.autocast(enabled=False):
            image = image.float()
            start_image = start_image.float()
            loss = lpips_loss_fn(image, start_image).mean()
...
def forward_train_step(
    ...
    if args.embed_guidance:
        ...
        w_embedding = None
    print('NOISE PRED DTYPE', noisy_model_input.dtype)
    noise_pred = unet(
        noisy_model_input,
        start_timesteps,
        timestep_cond=w_embedding,
    ...
    # Calculate loss
    loss_type = 'huber' if args.loss_type == 'lpips' else args.loss_type
    if loss_type == "l2":
        loss = F.mse_loss(model_pred.float(), target_pred.float(), reduction="mean")
...
def forward_preserve_train_step(
    ...
    # Calculate loss
    if loss_type == "l2":
        losses = (start_model_pred.float() - start_model_input.float()) ** 2
        # losses = F.mse_loss(
        #     start_model_pred.float(), start_model_input.float(), reduction="None"
        # )
    elif loss_type == "huber":
        losses = (
            torch.sqrt(
