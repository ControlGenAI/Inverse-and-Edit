    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            ...
            for log in image_logs:
                ...
                formatted_images = np.stack(formatted_images)
                logger.warn(formatted_images.shape)

                tracker.log({f"{log['prompt']}": wandb.Image(np.hstack(formatted_images), caption=validation_prompt)}, step=step)
        # else:
...
def log_train_inversion(
    ...
    validation_data = get_coco_loader(args, batch_size=num_validation_prompts, is_train=True)
    batch = next(validation_data)
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                validation_prompt = (
                    "TRAIN INVERSION: " + log["prompt"] + f" | CFG {guidance_scale + 1}"
                )
                formatted_images = []
                formatted_images.append(np.asarray(log["decoded_latent"]))
                formatted_images.append(np.asarray(log["orig_image"]))
...
def eval_inversion(
    ...
    if accelerator.is_main_process:
        ...
        if not is_train:
            ...
            logs = {
                "train_recon_loss_cm": float(np.mean(gathered_cm_losses)),
                # "train_fid_score_cm": fid_score_cm,
            }
